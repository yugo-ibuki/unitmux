...
import { listPanes, sendInput, capturePane, getPaneDetail, getPaneCwd, gitAdd, gitCommit, gitPush, listTmuxSessions, createSession, killPane } from './tmux'
import { listSlashItems } from './commands'
...
app.whenReady().then(() => {
  ...
  ipcMain.handle('commands:list', async (_event, target: string) => {
    try {
      ...
      return await listSlashItems(cwd)
    } catch {
      return []
    }
  })
